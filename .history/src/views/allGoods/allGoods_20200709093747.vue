...
export default {
  ...
  methods: {
    ...
    sortByPrice(v) {
      ...
      this.sort = v;
      this.$api.allGoodSort({page:this.page,size:this.size,sort:this.sort}).then(res =>{
        this.defaulted = res.data
      }).catch(err => {
        console.log(err);
      });
    ...
    search() {
      ...
      this.$api.allGoods({page:this.page, size:this.size, sort:this.sort, priceGt, priceLte})
    }
  },
  mounted() {
    this.getData();
